    ...
    def get(self, request, blogSlug, format=None):
        ...
        try:
            ...
            if Comment.objects.filter(blog=blog).exists():
                comments = Comment.objects.order_by("-created_at").filter(blog=blog)
...
class GetBlogCommentView(APIView):
    def get(self, request, blogSlug, format=None):
        ...
        try:
            ...
            return Response({"comment": result}, status=status.HTTP_200_OK)
        except:
            return Response(
...
class CreateBlogCommentView(APIView):
    def post(self, request, blogSlug, format=None):
        ...
        try:
            ...
            comment = Comment.objects.create(user=user, blog=blog, comment=comment)
            ...
            return Response(
                {"comment": result, "comments": results}, status=status.HTTP_201_CREATED
            )
        except:
            return Response(
